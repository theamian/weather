...
function clean() {
    ...
    if(document.querySelector(".unitContainer")) document.querySelector(".unitContainer").remove();
}

// function to call the weather API
async function getWeather(lat, lon, units="metric") {
    try {
        const response = await fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=${units}&exclude=minutely,hourly&appid=${APIK}`)
...
function displayInfo({name, weatherData, units}) {
    ...
    const lat = weatherData.lat, lon = weatherData.lon;
    ...
    rightColumn.innerText = `${weatherData.current.temp.toFixed(0)}°${units.temp}`;
    ...
    feelsDiv.innerText = `feels like: ${weatherData.current.feels_like.toFixed(0)}°${units.temp}`;
    ...
    windDiv.innerText =`wind: ${weatherData.current.wind_speed.toFixed(0)} ${units.speed} ${windDirection(weatherData.current.wind_deg)}`;
    ...
    for(day of weatherData.daily) {
        ...
        nightTempDiv.innerText = `night: ${day.temp.night.toFixed(0)}°${units.temp}`;
...
function whatstheTemp(weatherData, units) {
    ...
    if((units === "metric" && temp > 35) || (units === "imperial" && temp > 95)) {
        changeStyling("veryhot");
    } else if((units === "metric" && temp > 25) || (units === "imperial" && temp > 77)) {
        changeStyling("hot");
    } else if((units === "metric" && temp > 15) || (units === "imperial" && temp > 59)) {
        changeStyling("warm");
    } else if((units === "metric" && temp <= 5) || (units === "imperial" && temp <= 41)) {
        changeStyling("cold");
    }
}
